...
if "llm" not in st.session_state:
    st.session_state["llm"] = "huggingfaceh4/zephyr-7b-alpha"
#Loading the LLM model from HugginFace API
llm = HuggingFaceEndpoint(
    endpoint_url=st.session_state['llm'],
    max_new_tokens = 2048,
...
_template = """Given the following conversation and a follow up question, rephrase the follow up question to be a standalone question only if the question is relevent to the history 
else return the same question
Chat History:
{chat_history}
Question: {question}
Standalone question:"""
...
Return only the question no answer is needed
Query: {question}
...
if "questions" not in st.session_state:
    #st.session_state["questions"] = ["Scholarship Schemes for Girl Students in Tamil Nadu",
     #                                 "Central Government Schemes for Farmers", 
      #                                "Andhra Pradesh Government Schemes for UPSC aspirants", 
       #                               "Kerala Government Social Justice Department Schemes for disabled"]
...
with st.sidebar:
    st.title("Change the Model")
    st.session_state["llm"] = st.selectbox('Pick a Model', ('mistralai/Mistral-7B-Instruct-v0.2','HuggingFaceH4/zephyr-7b-beta' ))
    #inp = st.text_input("Enter the Scheme Name")
    #btn = st.button(label="Select Model")
    st.text(st.session_state["llm"])
